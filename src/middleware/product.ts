...
export default class ProductMiddleware{
    static async validateBody(req: Request, _: Response, next: NextFunction){
        ...
        if(!price) throw new AppError('Preço não fornecido', 400);
        if(!storeId) throw new AppError('Loja não fornecida', 400);
        if(!stock) throw new AppError('Quantidade em estoque não fornecida', 400);
